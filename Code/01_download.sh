...
# ============================================================
#        GSE279086 GEO Download + Cleanup Script
# ============================================================

# -------------------- CONFIGURATION -------------------------

# Mapping of GSM IDs to sample codes
declare -A samples=(
...
for GSM in "${!samples[@]}"; do
    ...
    DEST_DIR="$BASE_DIR/$GSM"

    mkdir -p "$DEST_DIR"

    echo "Processing $GSM ($SAMPLE)"
    ...
    for FILE in "${FILES[@]}"; do
        ...
        if [[ ! -f "${DEST_DIR}/${FILENAME}" ]]; then
            ...
            wget -q -P "$DEST_DIR" "$URL" || echo "   Failed: $FILENAME"
        else
    ...
    rm -f *_barcodes_processed.tsv.gz
    rm -f *_features_processed.tsv.gz
    rm -f *_matrix_processed.mtx.gz

    # Rename to 10X standard names
...
echo "All downloads and cleanup completed successfully!"
echo "Each GSM folder now contains:"
